...
function ytdubDetectEntry() {
  /**
   * Service worker: dùng `ServiceWorkerGlobalScope` — đáng tin cậy hơn `!globalThis.window`
   * (một số môi trường/polyfill gán window lên globalThis → nhầm SW với content → nạp sw-pack
   * trong trang → `chrome.webRequest` undefined → lỗi onCompleted).
   */
  try {
    if (
      typeof ServiceWorkerGlobalScope !== "undefined" &&
      globalThis.self instanceof ServiceWorkerGlobalScope
    ) {
  ...
  /** Fallback (engine cũ): trong SW thường không có window. */
  try {
