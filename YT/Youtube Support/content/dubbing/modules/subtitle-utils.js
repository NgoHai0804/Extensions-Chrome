(function ytdubSubtitleUtils() {
  ...
  core.createSubtitleUtils = function createSubtitleUtils(deps) {
    const {
      ...
      log,
      logPipeline = () => {},
      /** (url, signal?) → { ok, status, text, contentType } — ưu tiên SW để khớp URL đã bắt + fetch extension. */
    ...
    function timedtextUrlSnippet(u) {
      const s = String(u || "");
    ...
    async function fetchTimedtextCuesSerialized(abs, signal) {
      if (signal?.aborted) return [];
      logPipeline("timedtext | bắt đầu", timedtextUrlSnippet(abs));
      const urls = [abs];
      ...
      async function fetchTimedtextHttp(currentUrl) {
        if (typeof fetchTimedtextBodyViaSw === "function") {
          logPipeline("timedtext | fetch qua SW");
          return fetchTimedtextBodyViaSw(currentUrl, signal);
        }
      ...
      for (let u = 0; u < urls.length; u += 1) {
        ...
        for (let att = 0; att < MAX_TIMEDTEXT_ATTEMPTS; att += 1) {
          ...
          try {
            ...
            if (signal?.aborted) return [];
            const res = await fetchTimedtextHttp(currentUrl);
            if (!res.ok) {
              if (res.error) logPipeline("timedtext | lỗi mạng/SW:", String(res.error));
              logPipeline("timedtext | HTTP " + res.status, "urlIdx=" + u, "att=" + (att + 1));
              if (res.status === 429) {
                ...
                if (hadTlang) {
                  ...
                  logPipeline("timedtext | 429 → bỏ tlang, thử lại");
            ...
            if (cues.length) {
              logPipeline("timedtext | OK cues=" + cues.length, "urlIdx=" + u);
              return cues;
            }
            logPipeline("timedtext | parse 0 cue", "urlIdx=" + u, "bodyLen=" + body.length);
          } catch (e) {
            if (signal?.aborted) return [];
            lastErr = e;
            logPipeline("timedtext | lần lỗi:", String(e?.message || e));
          }
        }
      }
      if (lastErr) {
        logPipeline("timedtext | thất bại sau mọi fmt:", String(lastErr?.message || lastErr));
        log("timedtext fail (đã thử URL gốc + fmt)", String(lastErr?.message || lastErr));
      }
      return [];
    }
      ...
      while (Date.now() < deadline) {
        if (video?.textTracks?.length) {
          for (let i = 0; i < video.textTracks.length; i += 1) {
            ...
            if (part.length >= 1) {
              logPipeline("textTracks DOM | OK từ track i=" + i + " cues=" + part.length);
              return part;
            }
          }
        }
        if (video?.textTracks?.length) {
          ...
          if (best.length >= 1) {
            logPipeline("textTracks DOM | OK best cues=" + best.length);
            return best;
          }
        }
        await sleep(280);
      }
      logPipeline("textTracks DOM | hết thời gian, 0 cue");
      return [];
    }
    ...
    async function loadCuesFromCaptionTracks(tracks, opts) {
      const signal = opts?.signal;
      if (!tracks?.length) {
        logPipeline("captionTracks | không có track trong list");
        return null;
      }
      const seen = new Set();
      const sequence = [];
      ...
      for (let i = 0; i < sequence.length; i += 1) {
        ...
        logPipeline(
          "captionTracks | track " + (i + 1) + "/" + sequence.length,
          "lang=" + String(cap.languageCode || ""),
          "target=" + target
        );
        const candidates = [];
        ...
        for (let u = 0; u < candidates.length; u += 1) {
          if (signal?.aborted) return null;
          logPipeline("captionTracks | candidate URL " + (u + 1) + "/" + candidates.length);
          cues = await fetchTimedtextCues(candidates[u], opts);
          if (cues.length) break;
        }
        if (cues.length) {
          logPipeline("captionTracks | OK lang=" + String(cap.languageCode || ""));
          return { cues, lang: cap.languageCode || cap.name?.simpleText || "" };
        }
        logPipeline("captionTracks | track này 0 cue sau mọi candidate");
      }
      logPipeline("captionTracks | hết sequence, null");
      return null;
    }
