 ...
 *
 * Copy cây (bỏ mọi .js + thư mục entry) → esbuild entry/main-entry.js → obfuscator main.js → (terser nếu không selfDefending) → manifest.
 */
"use strict";
...
async function processMainJs() {
  ...
  obfuscateMain(abs);
  if (OBF_CONFIG_MAIN.selfDefending) {
    console.log("[main.js] obfuscator xong — bỏ qua terser (selfDefending không tương thích minify sau obfuscate)");
  } else {
    console.log("[main.js] terser sau obfuscator");
